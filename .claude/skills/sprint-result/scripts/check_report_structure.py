...
`resources/fact_gates.md`, что проверяется без знания ПЛАН:
  ...
  гейт 6 — строк на стрим не больше, чем читается с экрана (предупреждение).
...
MAX_ROWS_PER_STREAM = 8   # столько строк несёт самый плотный слайд эталона
    ...
    for header, rows, section in _tables(text):
        ...
        if len(filled) > MAX_ROWS_PER_STREAM:
            ...
            warnings.append(f"гейт 6: «{name}» — строк {len(filled)}, порог "
                            f"{MAX_ROWS_PER_STREAM}. Слайд вырастет, но таблица "
                            "перестанет читаться с экрана; разрез — решение PO")
    return warnings
