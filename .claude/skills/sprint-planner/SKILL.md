...
## Стык с отчётом
ФАКТ прошлого спринта собирает навык `sprint-result` (`/sprint-result` → `/sr-final`). Формат и что именно из него берётся — `skills/sprint-result/resources/handoff.md`. Velocity в ФАКТ не переносится: ёмкость на fallback, пока не закрыт вопрос О9 БФТ отчёта.
