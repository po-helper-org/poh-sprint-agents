...
## Источник фактов из JIRA
Если в среде есть инструменты `jira_current_stories` / `jira_search` / `jira_issue` (раздел «Управление спринтами», встроенный коннектор), факты о задачах — carryover, эпики, фактическая загрузка — берём оттуда и называем ключом задачи. Нет инструментов → работаем по волту. Правила и стадии — `resources/jira_connector.md`. Коннектор только читает.
...
## Границы (slice 1)
Всё в vault под git, обратимо. Запись в JIRA, презентация, аудио, cron — за границей среза: коннектор JIRA читает, но не пишет.
